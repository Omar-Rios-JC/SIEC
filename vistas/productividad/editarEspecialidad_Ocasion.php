...
if (!$id) {
    header('Location: Especialidad_Ocasion.php?error=id no válido');
    exit;
}

$esp = Especialidad_Ocasion::obtenerPorId($id);
?>
...
<script src="https://cdn.datatables.net/1.13.6/js/jquery.dataTables.min.js"></script>
    <title>Editor de registros | Especialidad de Ocasión | Administrador</title>
    <style>
    th,
    td {
...
<nav class="navbar navbar-expand-lg navbar-dark" style="background-color: #7a123a;">
    ...
    <div class="container mt-4">
        <div class="card p-4">
            <h4 class="text-center">Editor de registros - Especialidad de ocasión</h4>
           <form method="POST" action="../../controladores/Especialidad_Ocasion.php">
    ...
    <div class="mb-3">
        ...
        <input type="text" name="descripcion" class="form-control" value="<?= htmlspecialchars($esp[3]) ?>" required>
    </div>

    <div class="mb-3">
        <label class="form-label mb-1">Año:</label>
        <input type="number" name="anio" class="form-control" value="<?= htmlspecialchars($esp[28]) ?>" required>
    </div>
    ...
    <div class="row">
        ...
        $i = 4; // empiezan los datos mensuales a partir del índice 4
        foreach ($meses as $mes) {
        ?>
        <div class="col-6 col-md-3 mb-2">
            <label class="form-label mb-1"><?= ucfirst($mes) ?> 1era:</label>
            <input type="number" name="<?= strtolower($mes) ?>_1era" class="form-control"
                value="<?= htmlspecialchars($esp[$i++]) ?>" min="0">
        </div>
        <div class="col-6 col-md-3 mb-2">
            <label class="form-label mb-1"><?= ucfirst($mes) ?> Sub:</label>
            <input type="number" name="<?= strtolower($mes) ?>_sub" class="form-control"
                value="<?= htmlspecialchars($esp[$i++]) ?>" min="0">
        </div>
        <?php } ?>
    </div>
