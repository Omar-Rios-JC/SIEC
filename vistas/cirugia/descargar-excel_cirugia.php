...
$categoriasMeses = [new DataSeriesValues('String', "'Cirugia'!\$D\$1:\$O\$1", null, 12)];
    ...
    foreach ($agrupadoEspecialidad as $especialidad => $filasEspecialidad) {
        ...
        for ($i = 0; $i < 12; $i++) {
            ...
            foreach ($filasEspecialidad as $filaIndex) {
                $celulasMes[] = "Cirugia!{$colMes}{$filaIndex}";
            }
...
foreach ($datosFiltrados as $dato) {
    ...
    $celdaTotal = "Cirugia!P{$filaIndex}";
...
$chart2 = new Chart('chart2', new Title('Días Cirugia anual por división'), $legend, $plotArea2, true, 0, new Title('División'), new Title('Total'));
...
header('Content-Disposition: attachment;filename="tabla_y_graficos-Cirugias.xlsx"');
