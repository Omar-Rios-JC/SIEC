...
switch ($accion) {
    case 'Ingresar':
        if (
            ...
            if ($modo === 'insertar' && $urg->claveAnioExiste($urg->clave, $urg->anio)) {
                header('Location: ../vistas/hospitalizacion/ingresar-egreso.php?error=Ya existe esa clave para ese año');
                exit;
            }

            // Insertar nuevo si no existe, o actualizar campos vacíos si ya existe
            if ($urg->ingresar()) {
                header('Location: ../vistas/hospitalizacion/egresos_inicio.php?msg=Operación exitosa');
            } else {
                header('Location: ../vistas/hospitalizacion/egresos_inicio.php?error=No se realizó ningún cambio');
            }
            exit;
        } else {
            header('Location: ../vistas/hospitalizacion/egresos_inicio.php?error=Faltan datos');
            exit;
        }
        break;
    ...
    case 'Editar':
        if (!empty($_POST['id'])) {
            ...
            header('Location: ../vistas/hospitalizacion/egresos_inicio.php?msg=Editado');
            exit;
        }
        break;
    ...
    case 'Eliminar':
        if (!empty($_GET['id'])) {
            ...
            header('Location: ../vistas/hospitalizacion/egresos_inicio.php?msg=Eliminado');
            exit;
        }
        break;
    ...
    case 'ReemplazarCSVFiltrado':
        ...
        while (($datos = fgetcsv($archivo)) !== false) {
            ...
            $queryVerificar = "SELECT * FROM egresos WHERE $whereSQL";
    ...
    default:
        header('Location: ../vistas/hospitalizacion/egresos_inicio.php');
        exit;
}
