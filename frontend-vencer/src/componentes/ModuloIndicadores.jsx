...
import { Target, Calendar, PieChart, Activity, LayoutDashboard, Users, FileText, Filter, ClipboardList, ChevronLeft, ChevronRight, Menu } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-slate-50 md:flex md:h-screen md:overflow-hidden">
           ...
           {/* SIDEBAR MENÚ LATERAL COLAPSABLE */}
                ...
                <div className="fixed inset-0 z-50 md:hidden">
                    ...
                    <aside className="relative h-full w-[280px] max-w-[86vw] bg-white border-r border-slate-200 shadow-2xl flex flex-col">
                        ...
                        <nav className="flex-1 p-4 space-y-2">
                            <button
                                type="button"
                                onClick={() => { setSeccionSidebar('hosp'); setSidebarMovilAbierta(false); }}
                            ...
                            <button
                                type="button"
                                onClick={() => { setSeccionSidebar('consulta_externa'); setSidebarMovilAbierta(false); }}
            ...
            <aside className={`hidden md:flex ${sidebarColapsada ? 'w-20' : 'w-64'} bg-white border-r border-slate-200 flex-col z-20 shrink-0 transition-all duration-300 ease-in-out relative`}>
            ...
            <main className="flex-1 flex flex-col min-w-0 md:overflow-hidden relative">
                ...
                <header className="bg-white border-b border-slate-200 px-4 py-4 sm:px-6 lg:px-8 sm:py-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between shrink-0">
                    ...
                    <div className="grid w-full grid-cols-2 bg-slate-100 p-1 rounded-xl shadow-inner border border-slate-200 lg:w-auto">
                        <button onClick={() => setTabActiva('mensual')} className={`flex items-center justify-center gap-2 px-3 sm:px-6 py-2 rounded-lg text-sm font-bold transition-all ${tabActiva === 'mensual' ? "bg-white text-emerald-700 shadow-md" : "text-slate-400"}`}><Calendar size={16}/> Mensual</button>
                        <button onClick={() => setTabActiva('acumulado')} className={`flex items-center justify-center gap-2 px-3 sm:px-6 py-2 rounded-lg text-sm font-bold transition-all ${tabActiva === 'acumulado' ? "bg-white text-emerald-700 shadow-md" : "text-slate-400"}`}><PieChart size={16}/> Acumulado</button>
                    </div>
                </header>
                                ...
                                <div className="space-y-4 sm:space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
                                    ...
                                    <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-100 shadow-sm">
                                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div><h3 className="font-bold text-slate-800 text-base sm:text-lg">Cumplimiento de Metas Semanales</h3><p className="text-xs font-medium text-slate-500">Calendario Operativo IMSS</p></div>
                                            </div>
                                        ...
                                        <div className="h-64 sm:h-80 w-full"><Line data={chartMetas} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top', align: 'end' } }, scales: { y: { beginAtZero: true, grid: { color: '#f8fafc' } }, x: { grid: { display: false } } } }} /></div>
                                    </div>
                                    ...
                                    <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-100 shadow-sm">
                                        <div className="flex flex-col sm:flex-row sm:items-start gap-3 mb-6">
                                            ...
                                            <div>
                                                <h3 className="font-bold text-slate-800 text-base sm:text-lg">Productividad Diaria por Consultorio</h3>
