            ...
            y: {
              ticks: {
                ...
                callback: function (v) {
                  ...
                  return l.length > 25 ? l.substr(0, 25) + "..." : l;
                },
              },
              grid: { display: false },
            },
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden h-full flex flex-col transition-all duration-300">
      ...
      <div
        className={`flex-1 grid grid-cols-1 ${
          mostrarTablas ? "lg:grid-cols-5" : "lg:grid-cols-1"
        } min-h-[300px]`}
      >
        <div
          className={`p-4 col-span-1 ${
            mostrarTablas
              ? "lg:col-span-3 border-b lg:border-b-0 lg:border-r border-slate-100"
              : "lg:col-span-1"
          } transition-all`}
        >
          <div className="w-full h-full relative min-h-[250px]">
            {tipo === "bar" ? (
              ...
              <div className="h-full flex justify-center">
                <Doughnut data={chartData} options={options} id={idCanvas} />
              </div>
            )}
          </div>
        </div>
        ...
        {mostrarTablas && (
          <div className="bg-white col-span-1 lg:col-span-2 max-h-[300px] overflow-x-auto overflow-y-auto custom-scrollbar">
                ...
                {procesados.ordenados.map(([nombre, cantidad], i) => (
                  <tr key={i} className="hover:bg-slate-50 transition">
                    <td
                      ...
                      title={nombre}
                    >
                      <span className="text-[9px] text-slate-300 font-mono inline-block w-3 mr-1">
                        {i + 1}
                      </span>
                      {nombre}
                    </td>
...
// ============================================================================
// 2. COMPONENTE: ANÁLISIS DE CATEGORÍAS
// ============================================================================
const AnalisisCategorias = ({ idCanvas, datos, mostrarTablas = true }) => {
  const { chartData, tableData, totalTotal } = useMemo(() => {
    ...
    return {
      chartData: {
        labels: top5.map((i) =>
          i.nombre.length > 25 ? i.nombre.substring(0, 25) + "..." : i.nombre,
        ),
        datasets: [
          {
            label: "Adversos",
            data: top5.map((i) => i.adversos),
            backgroundColor: "#ef4444",
            borderRadius: 4,
          },
          {
            label: "Cuasifallas",
            data: top5.map((i) => i.cuasi),
            backgroundColor: "#f59e0b",
            borderRadius: 4,
          },
          {
            label: "Centinelas",
            data: top5.map((i) => i.centinelas),
            ...
            <table className="w-full text-[11px] text-left text-slate-600">
              <thead className="text-[10px] text-slate-400 uppercase bg-slate-50 sticky top-0 shadow-sm z-10">
                <tr>
                  ...
                  <th
                    className="px-2 py-2 text-center text-red-600"
                    title="Adversos"
                  >
                    Adv.
                  </th>
                  <th
                    className="px-2 py-2 text-center text-amber-600"
                    title="Cuasifallas"
                  >
                    Cuas.
                  </th>
                ...
                {tableData.map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-2 py-2 text-center font-bold text-slate-500">
                      {row.adversos > 0 ? row.adversos : "-"}
                    </td>
                    <td className="px-2 py-2 text-center font-bold text-slate-500">
                      {row.cuasi > 0 ? row.cuasi : "-"}
                    </td>
                    <td className="px-2 py-2 text-center font-bold text-slate-500">
                      {row.centinelas > 0 ? row.centinelas : "-"}
                    </td>
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <SeccionGraficoTabla
          idCanvas={`${prefijoId}_areas_mayor_incidencia`}
          titulo="Áreas con Mayor Incidencia"
          datos={datos}
          campo="servicio"
          color={colorArea}
        ...
        {topAreas[0] && (
          <div className="ring-2 ring-offset-2 ring-slate-100 rounded-xl">
            <SeccionGraficoTabla
              idCanvas={`${prefijoId}_analisis_area_1`}
              titulo={`#1 ANÁLISIS ÁREA: ${topAreas[0]}`}
              datos={datos.filter((d) => d.servicio === topAreas[0])}
              campo="definicion"
              color={colorCausa}
              limite={7}
              mostrarTablas={mostrarTablas}
      ...
      {(topAreas[1] || topAreas[2]) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {topAreas[1] ? (
            <SeccionGraficoTabla
              idCanvas={`${prefijoId}_analisis_area_2`}
              titulo={`#2 ANÁLISIS ÁREA: ${topAreas[1]}`}
              datos={datos.filter((d) => d.servicio === topAreas[1])}
              campo="definicion"
              color={colorCausa}
              limite={5}
              mostrarTablas={mostrarTablas}
          ...
          {topAreas[2] ? (
            <SeccionGraficoTabla
              idCanvas={`${prefijoId}_analisis_area_3`}
              titulo={`#3 ANÁLISIS ÁREA: ${topAreas[2]}`}
              datos={datos.filter((d) => d.servicio === topAreas[2])}
              campo="definicion"
              color={colorCausa}
              limite={5}
              mostrarTablas={mostrarTablas}
...
// ============================================================================
const AcordeonProcesos = ({
  prefijoId,
  datos,
  colorBase = "#005C46",
  titulo = "Análisis de Procesos Relacionados",
  mostrarTablas,
}) => {
  ...
  return (
    <div className="mt-8 mb-6">
      ...
      >
        ...
        {titulo}
        {expandido ? (
          <ChevronUp size={22} className="text-[#005C46]" />
        ) : (
          <ChevronDown size={22} className="text-[#005C46]" />
        )}
      </button>
      ...
      {expandido && (
        <div className="mt-6 space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SeccionGraficoTabla
              idCanvas={`${prefijoId}_procesos_mayor_incidencia`}
              titulo="Procesos con Mayor Incidencia"
              datos={datos}
              campo="proceso"
              color={colorBase}
              limite={10}
            ...
            {topProcesos[0] && (
              <div className="ring-2 ring-offset-2 ring-amber-200 rounded-xl">
                <SeccionGraficoTabla
                  idCanvas={`${prefijoId}_procesos_causas_1`}
                  titulo={`#1 CAUSAS: ${topProcesos[0]}`}
                  datos={datos.filter((d) => d.proceso === topProcesos[0])}
                  campo="definicion"
                  color="#D4C19C"
                  limite={7}
                  mostrarTablas={mostrarTablas}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {topProcesos[1] && (
              <SeccionGraficoTabla
                idCanvas={`${prefijoId}_procesos_causas_2`}
                titulo={`#2 CAUSAS: ${topProcesos[1]}`}
                datos={datos.filter((d) => d.proceso === topProcesos[1])}
                campo="definicion"
                color="#007A5E"
                limite={5}
                mostrarTablas={mostrarTablas}
            ...
            {topProcesos[2] && (
              <SeccionGraficoTabla
                idCanvas={`${prefijoId}_procesos_causas_3`}
                titulo={`#3 CAUSAS: ${topProcesos[2]}`}
                datos={datos.filter((d) => d.proceso === topProcesos[2])}
                campo="definicion"
                color="#007A5E"
                limite={5}
                mostrarTablas={mostrarTablas}
...
function dashboardVencer() {
  ...
  const moduloInicial =
    queryParams.get("modulo") === "productividad" ? "productividad" : "vencer";
  ...
  const generarExcelReporte = async () => {
    ...
    try {
      ...
      const addSection = (ws, title, obj, startRow, canvasIdParaFoto) => {
        ...
        ws.getCell(`A${startRow + 1}`).font = {
          color: { argb: "FFFFFFFF" },
        };

        ws.getCell(`B${startRow + 1}`).fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FF333333" },
        };
        ws.getCell(`B${startRow + 1}`).font = {
          color: { argb: "FFFFFFFF" },
        };
      ...
      const addSectionCategorias = (
        ...
        headers.forEach((h, i) => {
          ...
          ws.getCell(`${h}${startRow + 1}`).fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: "FF333333" },
          };
          ws.getCell(`${h}${startRow + 1}`).font = {
            color: { argb: "FFFFFFFF" },
          };
        });
        ...
        if (canvasIdParaFoto) {
          attachChartImage(ws, canvasIdParaFoto, 6, startRow);
        }
      ...
      datosFiltrados.forEach((r) => {
        ...
        if (!s.General.Categoria[cat]) {
          s.General.Categoria[cat] = {
            adversos: 0,
            cuasi: 0,
            centinela: 0,
            total: 0,
        ...
        if (evUpper.includes("ADVERSO")) s.General.Categoria[cat].adversos++;
          ...
          s.General.Categoria[cat].centinela++;
        }
        ...
        if (evUpper.includes("ADVERSO")) {
          ...
          s.Centinela.Definicion[def] =
            (s.Centinela.Definicion[def] || 0) + 1;
      ...
      rA = addSection(
        ...
        s.Adverso.Servicio,
        rA,
        "adv_areas_mayor_incidencia",
      );
      rA = addSection(
        wsA,
        "CAUSAS GLOBALES (Adversos)",
        s.Adverso.Definicion,
        rA,
        "adv_causas_globales",
      );
      rA = addSection(
        wsA,
        "PROCESOS RELACIONADOS (Adversos)",
        s.Adverso.Proceso,
        rA,
        "adv_procesos_mayor_incidencia",
      );
      ...
      rC = addSection(
        wsC,
        "TOP ÁREAS (Cuasifallas)",
        s.Cuasi.Servicio,
        rC,
        "cuasi_areas_mayor_incidencia",
      );
      rC = addSection(
        wsC,
        "CAUSAS GLOBALES (Cuasifallas)",
        s.Cuasi.Definicion,
        rC,
        "cuasi_causas_globales",
      );
      rC = addSection(
        wsC,
        "PROCESOS RELACIONADOS (Cuasifallas)",
        s.Cuasi.Proceso,
        rC,
        "cuasi_procesos_mayor_incidencia",
      );
      ...
      rCen = addSection(
        wsCen,
        "TOP ÁREAS (Centinelas)",
        s.Centinela.Servicio,
        rCen,
        "cen_areas_mayor_incidencia",
      );
      rCen = addSection(
        wsCen,
        "CAUSAS GLOBALES (Centinelas)",
        s.Centinela.Definicion,
        rCen,
        "cen_causas_globales",
      );
      rCen = addSection(
        wsCen,
        "PROCESOS RELACIONADOS (Centinelas)",
        s.Centinela.Proceso,
        rCen,
        "cen_procesos_mayor_incidencia",
      );
      ...
      saveAs(
        new Blob([buffer]),
        `Reporte_VENCER_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
    } catch (error) {
      console.error(error);
        ...
        ...new Set(
          datos
            .map(
              (d) =>
                d.anio ||
                (d.fecha_evento ? d.fecha_evento.split("-")[0] : null),
            )
            .filter((a) => a),
        ),
      ]
    ...
    () =>
      datos.filter((item) => {
        ...
        const pasaServicio =
          ...
          (item.servicio || "Sin Dato").trim() === servicioSeleccionado;

        const pasaProceso =
          procesoSeleccionado === "todos" ||
          (item.proceso || "Sin Dato").trim() === procesoSeleccionado;
  ...
  const dAdversos = datosFiltrados.filter((d) =>
    (d.evento || "").toUpperCase().includes("ADVERSO"),
  );

  const dCuasi = datosFiltrados.filter((d) =>
    (d.evento || "").toUpperCase().includes("CUASI"),
  );

  const dCentinela = datosFiltrados.filter((d) =>
    (d.evento || "").toUpperCase().includes("CENTINELA"),
  );

  if (cargando) {
    return (
      <div className="h-screen flex flex-col items-center justify-center text-[#005C46] bg-slate-50 font-bold text-xl">
        <Activity className="animate-spin mb-4" size={40} />
        Cargando Sistema VENCER...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="h-screen flex items-center justify-center text-red-500 bg-slate-50 font-bold">
        {error}
      </div>
    );
  }
  ...
  return (
    <div
      className="text-slate-800 font-sans min-h-screen bg-slate-50 md:grid"
      style={{
    ...
    >
      {sidebarMobileOpen && (
        <div className="fixed inset-0 z-[100] md:hidden">
          ...
          <div className="relative h-full w-[280px] max-w-[85vw] bg-[#005C46] shadow-2xl">
            ...
            <Sidebar
              ...
              conteos={{
                general: dGeneral.length,
                adversos: dAdversos.length,
                cuasi: dCuasi.length,
                centinela: dCentinela.length,
              }}
              ...
              onVolverInicio={volverMenuPrincipal}
              onCerrarMovil={() => setSidebarMobileOpen(false)}
            />
          </div>
        </div>
      )}
      ...
      <div
        ...
        style={{ backgroundColor: "#005C46", zIndex: 20 }}
      >
        <Sidebar
          moduloActual={moduloActual}
          setModuloActual={setModuloActual}
      ...
      >
        {moduloActual === "vencer" && (
          <>
            <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm min-h-[64px] py-2 flex-shrink-0">
              <div className="flex flex-wrap items-center gap-2 px-4 md:px-6">
                <button
                  ...
                  className="md:hidden flex items-center justify-center rounded-xl bg-[#005C46] p-2.5 text-white transition-colors hover:bg-[#004a38]"
              ...
              <div className="max-w-[1400px] mx-auto px-4 md:px-6 h-full flex flex-col md:flex-row items-center justify-between gap-2">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 text-[#005C46] font-bold">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 bg-slate-50 rounded-lg p-1.5 border border-slate-200 text-sm shadow-inner flex-wrap justify-center">
                    ...
                    <select
                      className="bg-transparent font-bold text-[#005C46] outline-none cursor-pointer"
                    ...
                    <span className="font-bold text-slate-500 text-[10px] uppercase hidden sm:inline">
                      Servicio:
                    </span>

                    <select
                      className="bg-transparent font-bold text-[#005C46] outline-none cursor-pointer max-w-[120px] lg:max-w-[150px] truncate hidden sm:inline"
            ...
            <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 md:pt-8 pb-16 w-full transition-all duration-300 relative">
              {/* PESTAÑA: PANORAMA GENERAL */}
              <div
                className={
                  pestanaActiva === "general"
                    ? "block animate-in fade-in slide-in-from-bottom-4 duration-500"
                    : "absolute top-[-9999px] left-[-9999px] w-[1200px] invisible opacity-0"
                }
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                  <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 text-center">
                    ...
                    <p className="text-4xl font-black text-[#005C46] mt-2">
                      {dGeneral.length}
                    </p>
                  </div>
                ...
                <AcordeonProcesos
                  ...
                  datos={dGeneral}
                  colorBase="#005C46"
                  titulo="Análisis de Procesos Relacionados (General)"
                  mostrarTablas={mostrarTablas}
                />
              </div>
              ...
              >
                ...
                {dAdversos.length === 0 ? (
                  <div className="text-center p-10 text-slate-400 font-bold">
                    Sin registros de Eventos Adversos con estos filtros.
                  </div>
                ) : (
                  <>
                    {servicioSeleccionado !== "todos" && (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        ...
                        <SeccionGraficoTabla
                          idCanvas="adv_sexo"
                          titulo={`Distribución por Sexo en ${servicioSeleccionado}`}
                          datos={dAdversos}
                          campo="sexo"
                          tipo="doughnut"
                          color={["#ef4444", "#fca5a5", "#b91c1c"]}
                          mostrarTablas={mostrarTablas}
                        />
                      </div>
                    )}
                    ...
                    <div className="mb-6">
                      <SeccionGraficoTabla
                        idCanvas="adv_causas_globales"
                        titulo="Causas Globales (Definición)"
                        datos={dAdversos}
                        campo="definicion"
                        color="#991b1b"
                        limite={10}
                    ...
                    <AcordeonProcesos
                      ...
                      datos={dAdversos}
                      colorBase="#ef4444"
                      titulo="Procesos Relacionados (Adversos)"
                      mostrarTablas={mostrarTablas}
                    />
                  </>
                )}
              ...
              >
                <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl mb-6 flex gap-3 items-center text-amber-800 shadow-sm">
                  <ShieldAlert />
                  <h2 className="font-bold text-xl uppercase tracking-wider">
                    Análisis de Cuasifallas
                  </h2>
                </div>

                {dCuasi.length === 0 ? (
                  <div className="text-center p-10 text-slate-400 font-bold">
                    Sin registros de Cuasifallas con estos filtros.
                  </div>
                ) : (
                  <>
                    {servicioSeleccionado !== "todos" && (
                    ...
                    <div className="mb-6">
                      <SeccionGraficoTabla
                        idCanvas="cuasi_causas_globales"
                        titulo="Causas Globales (Definición)"
                        datos={dCuasi}
                        campo="definicion"
                        color="#b45309"
                    ...
                    <AcordeonProcesos
                      ...
                      datos={dCuasi}
                      colorBase="#f59e0b"
                      titulo="Procesos Relacionados (Cuasifallas)"
                      mostrarTablas={mostrarTablas}
                    />
                  </>
                )}
              ...
              >
                ...
                {dCentinela.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-16 bg-white border-2 border-dashed border-slate-200 rounded-2xl text-slate-400">
                    ...
                    <p className="text-sm">
                      No se encontraron registros críticos con los filtros
                      seleccionados.
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="mb-6">
                      <SeccionGraficoTabla
                        idCanvas="cen_causas_globales"
                        titulo="Causas Principales (Definición)"
                        datos={dCentinela}
                        campo="definicion"
                        color="#0f172a"
                        limite={10}
                    ...
                    <AcordeonProcesos
                      ...
                      datos={dCentinela}
                      colorBase="#475569"
                      titulo="Procesos Relacionados (Centinelas)"
                      mostrarTablas={mostrarTablas}
                    />
                  </>
                )}
