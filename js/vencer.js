document.addEventListener('DOMContentLoaded', function () {
    ...
    const cargarDatos = async () => {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                ...
                throw new Error(data.message || "Formato de datos no válido.");
            }
        } catch (error) {
            console.error("Error al cargar datos:", error);
            if (loader) loader.innerHTML = `<div class="alert alert-danger m-3">No se pudieron cargar los datos: ${error.message}</div>`;
        }
    };
    ...
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        ...
        const elMesIni = document.getElementById('filtroMesInicio');
        const elMesFin = document.getElementById('filtroMesFin');

        // Si los selectores de fecha/año no existen en la vista, mostrar la fila
        if (!elAnio && !elMesIni && !elMesFin) {
            return true;
        }

        const row = window.dbDatos[dataIndex] || {};
        const anioSel = elAnio ? elAnio.value : 'todos';
        const mesIni = elMesIni ? (parseInt(elMesIni.value) || 1) : 1;
        const mesFin = elMesFin ? (parseInt(elMesFin.value) || 12) : 12;

        let okAnio = (!anioSel || anioSel === 'todos' || String(row.anio) === anioSel);
        let okMes = true;

        if (okAnio && row.fecha_evento) {
            let m = 0;
            let f = String(row.fecha_evento);
            if (f.includes('-')) m = parseInt(f.split('-')[1]);
            else if (f.includes('/')) m = parseInt(f.split('/')[1]);

            if (m > 0 && (m < mesIni || m > mesFin)) {
                okMes = false;
            }
        }
